...
- **Resource** — a doctor/stylist/chair (belongs to a Business), working hours.
...
### Phase 3 — Booking API ← YOU ARE HERE
- Endpoints: list available slots, create booking, get booking by ID.
- Slot generation logic from working hours minus existing bookings (moved here from Phase 2).
- Handle double-booking with DB transactions/row locking.
- Idempotent booking creation (retry-safe).
- **Interview angle:** concurrency control, transactions, idempotency keys.
